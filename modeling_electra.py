  ...
  def __init__(self,
    ...
    if input_reprs is None:
      if input_embeddings is None:
        with tf.variable_scope((scope if untied_embeddings else "electra") + "/embeddings", reuse=tf.AUTO_REUSE):
...
def attention_layer_modified(from_tensor,
                    to_tensor,
                    attention_mask=None,
                    size_per_head=512):
